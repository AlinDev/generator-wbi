...
import { ALIN_BOGDAN_API, alinBogdanFailure, alinBogdanSuccess } from "../alinBogdan.actions";
...
const successActions = (response) => {
  return [alinBogdanSuccess(response),hideLoading({id:"alinBogdan"})];
};
const failActions = (error) => {
  return [alinBogdanFailure(error),hideLoading({id:"alinBogdan"})];
};
